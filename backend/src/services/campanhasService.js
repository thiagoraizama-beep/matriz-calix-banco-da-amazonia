...
export async function upsertCampanhaSheet(campanhaId, { spreadsheetId, range, mapping }) {
  const {
    data, campanha, campaignName, plataforma, vendedor, adName, adGroup, nomeCriativo, imagemCriativo,
  ...
  const { rows } = await query(
    `INSERT INTO campanha_sheets (
      ...
      col_data, col_campanha, col_campaign_name, col_plataforma, col_vendedor, col_ad_name, col_ad_group,
    ...
    ON CONFLICT (campanha_id) DO UPDATE SET
      ...
      col_campaign_name = EXCLUDED.col_campaign_name,
    ...
    [
      ...
      data, campanha || null, campaignName || null, plataforma, vendedor || null, adName || null, adGroup || null,
